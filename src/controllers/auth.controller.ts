...
export const login = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!userFound) {
      //Si no se encuentra el usuario retornamos la respuesta esta
    ...
    if (!isMatch) {
      //Si no matchean las password retorna error
    ...
    // quiero setear el token como cookie en el front para que se guarde en el navegador y no se pierda al recargar la pagina
    res.cookie("token", token, { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax"}); // guarda el token en una cookie, y lo enviamos en el header seteado como cookie.
    return res.json({
      id: userFound._id,
...
export const crud = async (req: Request, res: Response) => {
  //esta funcion autentica que el usuario este logueado
...
export const verifyToken = async (req: Request, res: Response) => {
  ...
  jwt.verify(
    token,
    TOKEN_SECRET,
    async (error: jwt.VerifyErrors | null, user: any) => {
      ...
      return res.json({
        // Si el usuario es encontrado, retorna el usuario
        id: userFound._id,
        username: userFound.username,
        email: userFound.email,
      });
